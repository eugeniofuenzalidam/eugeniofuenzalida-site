...
if (!parsed.success) return res.status(400).json({ success:false, error:'Invalid request data' });
```
...
Smart Choice:
```js
await db.collection('orders').doc(orderId).update({ mpPreferenceId: result.id, updatedAt: new Date().toISOString() });
```
